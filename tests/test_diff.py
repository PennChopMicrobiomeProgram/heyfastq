...
def test_diff_from_reads():
    a = Read("x", "CGCG", ".....")
    b = Read("x", "ATCGCGTC", ".........")
    assert FastqDiff.from_reads(a, b) == FastqDiff("x", "CGCG", "ATCGCGTC", -2)

def test_diff_format_tsv():
    d = FastqDiff("x", "ACGTG", "CAA", 1)
    assert d.format_tsv() == "x\tA\t\tG\t\t2,G,A;3,T,A\n"
